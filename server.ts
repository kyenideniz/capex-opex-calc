...
import { parseCJI3Excel } from './server/parser.js';
import { calculateWBSMonthlySummary, getTransactionMonth } from './server/deltaEngine.js';
import {
  generatePlaniswareExportExcel,
  generateReconciliationExportExcel,
} from './server/excelExport.js';
...
async function startServer() {
  ...
  app.post('/api/snapshots/commit', upload.single('file'), (req, res) => {
    try {
      const reportingMonth = req.body.reportingMonth; // "YYYY-MM"
      ...
      // Auto-register any WBS from file into projects table if missing
      ...
      // Register new snapshot & transactions
  ...
  app.get('/api/pm/summary', (req, res) => {
    ...
    // Transactions up to current requested reporting month
    const currentTx = db.transactions.filter((t) => getTransactionMonth(t) <= month);

    // Transactions before requested reporting month
  ...
  app.post('/api/pm/process', (req, res) => {
    ...
    // Calculate current confirmed amount
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`Planisware CAPEX Maintenance server running on http://localhost:${PORT}`);
  });
}
